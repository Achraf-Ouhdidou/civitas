...
export const getQuestions = (req, res) => {
  ...
  try {
    const questions = getQuestionsByLanguage(type, lang);
    res.json({ 
      questions, 
...
export const submitQuiz = async (req, res) => {
  const { answers, quizType = 'short', userId } = req.body;
...
export const getQuizHistory = async (req, res) => {
  ...
  try {
    ...
    res.json({ quizHistory: user.quizResults });
  } catch (error) {
    res.status(500).json({ error: 'Failed to fetch quiz history' });
  }
};

function getQuestionsByLanguage(type, lang) {
  const questionsData = {
    en: [
